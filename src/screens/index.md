...
| `CameraPreviewScreen.tsx` | file | Renders the full-screen live camera preview plus the tracking overlay, camera controls, and BLE status badge for an already-resolved `CameraDevice`. Takes `device`, `frameOutput`, `videoOutput`, `primary` (the already-decided `PrimaryAthleteResult`, not raw `boxes` — see 2026-08-16 note below), `frameAspectRatio`, `detectionStatus`, `facing`/`onToggleFacing`, `recordingStatus`/`saveStatus`/`onStartRecording`/`onStopRecording`, `bleState` as props. | ⚠️ needs verification — Stage 4 implemented (`docs/PRD.md` §4), `tsc`/tests pass, but never rendered inside the real RN/VisionCamera pipeline on a device. |
...
only), `src/theme/colors.ts`, `src/tracking/` (`computeTrackingReadout`,
`PrimaryAthleteResult` — `TrackingOverlay.tsx` only; it no longer calls `selectPrimaryAthlete`
itself, see 2026-08-16 note above), `src/hooks/useAthleteDetection.ts`
