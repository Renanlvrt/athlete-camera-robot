...
| `BleStatusBadge.tsx` | file | New 2026-08-14. Small always-visible label (top-left) showing the robot BLE link's state (`OFF`/`SCANNING…`/`CONNECTED`/`LOST`/etc). Pure rendering over one prop. | ⚠️ needs verification — `tsc` passes, untested on-device (needs a real BLE link to see any state but `'waiting-for-bluetooth'`). |
