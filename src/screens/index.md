...
| `TrackingOverlay.tsx` | file | Draws the locked athlete's box + confidence badge (clamped on-screen), a dashed line from box-center to screen-center, the distance/bearing/centred readout panel (including a compact up-down/left-right vector breakdown), and (2026-08-16, later still, TEMPORARY) a dashed red `rawUncorrectedBoxes` comparison box. Takes `primary` (a `PrimaryAthleteResult`) as a prop rather than `boxes` + deciding itself (changed 2026-08-16 — see below). Pure rendering over props. | ⚠️ needs verification on-device — three straight rounds of real-device box-position bugs (2026-08-14 both axes wrong; 2026-08-16 `orientBox` left/right swap fix, still wrong; 2026-08-16 later, AFTER an unrelated real aspect-ratio bug was also found and fixed, real-world left/right motion still shows up as vertical motion on screen AND in the BLE gimbal correction). The diagnostic red box exists specifically to stop guessing a fourth rotation formula — see `docs/VERIFICATION_REPORT.md`'s 2026-08-16/17 entries. Its decode/readout/box-placement math (the non-rotation parts) is validated against the real bundled model and real people via `.claude/skills/webcam-detection-preview/`. |
