...
As of **2026-08-09**, the only verified claims in this repo are that
`npm install` and `npm run typecheck` succeed. No CI build has run, no app
...
| `metro.config.js` | file | Needed before `.tflite` models can be bundled | 🔜 planned (does not exist yet) |
